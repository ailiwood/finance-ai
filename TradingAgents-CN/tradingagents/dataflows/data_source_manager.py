...
class DataSourceManager:
    ...
    def _get_tushare_stock_info(self, symbol: str) -> Dict:
        """使用Tushare获取股票基本信息（已付费用户的稳定数据源）"""
        try:
            from .providers.china.tushare import get_tushare_provider
            provider = get_tushare_provider()
            if not provider.api:
                # Try to init with token
                token = (
                    self._get_token_from_config("tushare") or
                    os.getenv("TUSHARE_TOKEN") or
                    os.getenv("tushare_token")
                )
                if token:
                    import tushare as ts
                    provider.api = ts.pro_api(token)
            ...
            # Convert symbol format: 600519 → 600519.SH, 000001 → 000001.SZ
            if symbol.startswith("6"):
                ts_symbol = f"{symbol}.SH"
            elif symbol.startswith(("0", "3")):
                ts_symbol = f"{symbol}.SZ"
            elif symbol.startswith(("8", "4")):
                ts_symbol = f"{symbol}.BJ"
            else:
                ts_symbol = symbol

            # Get stock basic info
            df = provider.api.stock_basic(
                ts_code=ts_symbol, fields="ts_code,name,area,industry,list_date"
            )
            if df is not None and not df.empty:
                row = df.iloc[0]
                logger.info(f"✅ [Tushare股票信息] {symbol} → {row['name']}")
                return {
                    "symbol": symbol,
                    "name": row["name"],
                    "area": row.get("area", ""),
                ...
                logger.warning(f"⚠️ Tushare 未找到 {ts_symbol} 的基本信息")
                return {"symbol": symbol, "name": f"股票{symbol}", "source": "tushare"}
