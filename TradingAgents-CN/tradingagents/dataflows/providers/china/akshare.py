...
class AKShareProvider(BaseStockDataProvider):
    ...
    def _initialize_akshare(self):
        ...
        try:
            ...
            # curl_cffi can be disabled via env var (fixes eastmoney connection issues)
            _curl_disabled = os.getenv("AKSHARE_CURL_CFFI_DISABLED", "") == "1"
            if _curl_disabled:
                use_curl_cffi = False
                logger.info("🔧 curl_cffi 已被 AKSHARE_CURL_CFFI_DISABLED 禁用，使用标准 requests")
            else:
                try:
    ...
    async def _get_realtime_quotes_data(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            def fetch_individual_spot():
                return self.ak.stock_zh_a_hist(symbol=code, period="daily", adjust="qfq")  # 前复权，与行情软件一致
