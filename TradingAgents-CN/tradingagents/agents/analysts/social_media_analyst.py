...
⚠️🚨 数据真实性红线（最高优先级，违反即为严重错误）：
- 如果未能从工具获取到真实的情绪/新闻数据，必须明确输出：
  "❌ 情绪数据暂不可用（数据源未配置或获取失败），无法进行分析。"
- 严禁编造情绪指数、评分、百分比或任何量化数据。
- 严禁在没有真实数据支撑的情况下声称"投资者情绪乐观/悲观"。
- 只有成功获取真实数据后，才能基于数据给出分析。

请撰写中文分析报告。如数据不可用，仅输出❌报错信息，不编造内容。"""
        )
