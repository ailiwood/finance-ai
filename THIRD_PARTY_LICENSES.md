...
| 依赖 | 许可证 | 用途 | 阶段 |
|------|--------|------|------|
| PyInstaller | GPL 2.0+ (with Bootloader Exception) | Windows .exe 打包 | M7 |
| Inno Setup | zlib/libpng (pre-6.5) / Commercial (6.5+) | Windows 安装程序 | M7 |
...
| 依赖 | 版本 | 许可证 | 用途 | 出处 |
|------|------|--------|------|------|
| PyInstaller | 6.x | GPL 2.0+ with Bootloader Exception | Python → Windows .exe 打包 | https://pyinstaller.org/ |
| Inno Setup | 6.3+ | zlib/libpng (免费) / Commercial (6.5+) | Windows 安装程序制作 | https://jrsoftware.org/isinfo.php |

**PyInstaller Bootloader Exception 说明**：PyInstaller 本身为 GPL，但其 Bootloader Exception 明确允许打包输出的 .exe 以任意许可证分发。打包后的 QuantSage.exe 不继承 GPL 许可证。详见 https://pyinstaller.org/en/stable/license.html
